...
const GoogleAuthButton = ({ mode = "signup", onError }) => {
  ...
  const shapeClasses = isLoginRoute ? "rounded-lg" : "rounded-full";

  // 👉 Switch to implicit flow so we get an access_token right away,
  // then fetch profile (email/name/picture/sub).
  const startGoogle = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const idToken = (tokenResponse)?.id_token; // may exist depending on config
