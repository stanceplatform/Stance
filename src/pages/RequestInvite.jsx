...
import TextField from '../components/ui/TextField';   // 👈 use the reusable input
...
const RequestInvite = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res?.success) {
        setOk(res?.message || 'Request submitted successfully.');
        setEmail('');
      } else {
    ...
    >
      ...
      <form id="inviteForm" onSubmit={submit} className="w-full max-w-[360px] pb-20">
        <TextField
          ...
          error={err}                 // shows red border + message if present
          helperText={ok}             // shows success copy (green style below)
        />

        {/* Optional: differentiate success color */}
        {ok ? <div className="mt-3 text-center text-[13px] text-green-200">{ok}</div> : null}
        {/* If you prefer ONLY inline helper/error in TextField, remove the two divs above/below */}
